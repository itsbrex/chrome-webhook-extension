...
async function extractLinkedInProfileAndSend(webhookUrl, urlToSend, tabId) {
  try {
    ...
    chrome.storage.local.get(
      ...
      async (settings) => {
        if (
          settings.linkedinAutoMutualConnections &&
          profileData.mutualConnectionsUrl
        ) {
          try {
...
function extractDataAndSendBasic(
  webhookUrl,
  urlToSend,
  type,
  tabId,
...
function showQueueNotification(webhookUrl, webhookName) {
  ...
  chrome.storage.local.get(
    ...
    (data) => {
      ...
      setTimeout(() => clearQueueNotification(webhookUrl), 60_000);
    }
  );
}
...
async function handleLinkedInMutualConnections(tab) {
  try {
    ...
    if (!(detectionResult.success && detectionResult.data)) {
      showNotification(
        '❌ LinkedIn Parser',
        'No mutual connections found on this page',
        false
      );
      return;
    }
    ...
    showNotification(
      '🔍 LinkedIn Parser',
      `Found ${mutualConnectionsData.totalCount || 'some'} mutual connections. Starting parsing...`,
      true
    );
          ...
          if (settings.linkedinBidirectional) {
            ...
            if (parseResult.success) {
              ...
              const results = await Promise.allSettled(
                parseResult.data.map((payload) =>
                  sendLinkedInDataToWebhooks(payload)
                )
              );
              ...
              if (failed.length === 0) {
                showNotification(
                  '✅ LinkedIn Parser',
                  `Successfully parsed ${parseResult.data.length} bi-directional payloads`,
                  true
                );
              } else {
                showNotification(
                  '⚠️ LinkedIn Parser',
                  `Parsed ${parseResult.data.length} payloads, but ${failed.length} failed to send`,
                  false
                );
            ...
            if (parseResult.success) {
              ...
              showNotification(
                '✅ LinkedIn Parser',
                `Successfully parsed ${parseResult.data.totalCount} mutual connections`,
                true
              );
            } else {
              throw new Error(
                parseResult.error || 'Failed to parse mutual connections'
              );
            }
          }
        } catch (error) {
...
async function handleLinkedInMutualConnectionsAuto(tabId, profileData) {
  try {
    ...
    showNotification(
      ...
      'Auto-parsing mutual connections...',
      true
    );
          ...
          if (settings.linkedinBidirectional) {
            ...
            if (parseResult.success) {
              ...
              if (failed.length === 0) {
                showNotification(
                  '✅ LinkedIn Auto Parser',
                  `Auto-parsed ${parseResult.data.length} bi-directional connections`,
                  true
                );
              } else {
                showNotification(
                  '⚠️ LinkedIn Auto Parser',
                  `Auto-parsed ${parseResult.data.length} connections, but ${failed.length} failed to send`,
                  false
                );
                failed.forEach((fail, idx) => {
                  console.error(
              ...
              throw new Error(
                parseResult.error || 'Failed to auto-parse mutual connections'
              );
            }
          } else {
            // Parse with single payload
            ...
            if (parseResult.success) {
              ...
              showNotification(
                '✅ LinkedIn Auto Parser',
                `Auto-parsed ${parseResult.data.totalCount} mutual connections`,
                true
              );
            } else {
              throw new Error(
...
async function sendLinkedInDataToWebhooks(linkedinData) {
  try {
    ...
    if (!data.webhooks || data.webhooks.length === 0) {
      showNotification(
        '⚠️ LinkedIn Parser',
        'No webhooks configured. Please add webhooks to receive LinkedIn data.',
        false
      );
      return;
    }
    ...
    if (targetWebhooks.length === 0) {
      showNotification(
        '⚠️ LinkedIn Parser',
        'No webhooks selected for LinkedIn data.',
        false
      );
      return;
    }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'linkedinMutualConnectionsDetected') {
    ...
    showNotification(
      '🔍 LinkedIn Parser',
      'Mutual connections detected on this page',
      true
    );
